...
export class FourHoursCandle_FetchesProcessor {
    constructor(
        ...
        @InjectQueue("FourHoursCandle_Calculations") private readonly calculcationsQueue: Queue,
    ) {
...
export class FourHoursCandle_CalculationsProcessor {
    ...
    async jobProcessor(job: Job) {
        const interval = Interval.FourHour;
    ...
    private async calculateRSI14Prerequisites(symbol: Symbol, candle: Candle, interval: Interval, symbolMeta: SymbolMeta) {
        try {
            ...
            if (!candles || candles.length < 14) {
                ...
                return false;
                // const startTime = await this.candleService.calculateStartTimeDependingOnTheLatestExistingCandle(symbol.symbol, interval);
                // const storedCandles = await this.candleService.fetchCandles(symbol.symbol, interval, startTime, 14 - candles.length);
                // if (storedCandles.length + candles.length < 14) {
                //     this.logger.error(`symbol haven't enough candles to calculate RSI14 prerequisites. interval: ${interval}, symbol: ${candle.symbol}`);
                //     return false;
                // }
            }

            // candles = await this.candleService.getCandles(symbol.symbol, interval, 14, "closeTime", "ASC");
            // if (!candles) return null;
...
@Processor('FourHoursCandle_Preprocessings')
@Injectable()
export class FourHoursCandle_PreprocessingsProcessor {
    constructor(
        private readonly symbolService: SymbolService,
        private readonly candleService: CandleService,
        private readonly candleMetaService: CandleMetaService,
        @InjectQueue("FourHoursCandle_Calculations") private readonly fourHoursCandleCalculationsQueue: Queue,
    ) {
        this.symbolService.setExchange(Exchanges.Binance);
        this.candleService.setExchange(Exchanges.Binance);
        this.candleMetaService.setExchange(Exchanges.Binance);
    ...
    private readonly logger = new Logger(FourHoursCandle_PreprocessingsProcessor.name);
    ...
    async jobProcessor(job: Job) {
        const symbol: Symbol = job.data.symbol;
        const interval = Interval.FourHour;

        let candles = await this.candleService.getCandles(symbol.symbol, interval, null, 'closeTime', "ASC");
        if (!candles) return null;


        // movaghat
        // note: candles as let beshe const.
        if (candles.length > 16) candles = candles.slice(0, 16)
        // end movaghat

        for (const candle of candles) {
            await this.fourHoursCandleCalculationsQueue.add("default_queue", { candle: candle });
        }
